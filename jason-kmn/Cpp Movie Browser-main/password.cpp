...
void PasswordTextfield::draw()
{
	...
	graphics::drawText(getX() - (2 * width / 5) - 5, getY() + 4, width / 20, space_for_text, brush_for_text);
	//------------------------------------------------------------------------------------------------------------
}

void PasswordTextfield::update()
{
			...
			if (graphics::getKeyState((graphics::scancode_t)i)) //we check if the proper character was pressed
			{
				...
				else if (delay >= 150.0f) //if we are ready to press another character
				{
					...
					if (text_inside.size() < 60) //if we have space to write more characters inside the textfield
					{
						...
						{
							text_inside.push_back('*'); //we add the key's character to textfield
...
void PasswordTextfield::init()
{
}

PasswordTextfield::PasswordTextfield(std::string new_name, float x, float y, float w, float h, std::string text)
{
	setName(new_name);
	setX(x);
	setY(y);
...
PasswordTextfield::~PasswordTextfield()
{
	...
	delete_message += " Password destructor called ";
